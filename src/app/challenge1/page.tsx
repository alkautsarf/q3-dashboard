...
import { fetchIndividualBalancesViem, fetchBalancesMulticallViem, fetchNativeToken, discoverOwnedTokens, mergeTokenData, isLikelyValidToken, fetchSmartContractBalances } from "@/app/lib/portfolio";
...
function Approach2({ address, net, hideSpam }: { address: string; net: NetworkKey; hideSpam: boolean }) {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const chooseBatchSize = (n: number) => (n <= 12 ? n : n <= 60 ? 40 : n <= 200 ? 100 : 120);
...
function Approach3({ address, net, hideSpam }: { address: string; net: NetworkKey; hideSpam: boolean }) {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const elapsedMs = Math.round(performance.now() - t0);
      const chooseBatchSize = (n: number) => (n <= 12 ? n : n <= 60 ? 40 : n <= 200 ? 100 : 120);
      const approxRpc = 1 /* discovery */ + Math.max(1, Math.ceil(nonSpam.length / chooseBatchSize(nonSpam.length))) /* contract batches */ + 1 /* native */;
      return { tokens: finalTokens, perf: { ms: elapsedMs, rpc: approxRpc } } as const;
    },
  });
    ...
    <div>
      ...
      {isLoading ? (
        ...
        <>
          {data?.perf && (
            <div className="mb-2 flex justify-end">
              <p className="text-xs text-gray-600">Approach: Smart Contract — {data.perf.ms}ms, ~{data.perf.rpc} RPC</p>
            </div>
          )}
          <TokensList items={items} loadingPrices={isFetchingErc20 || isFetchingNative} />
        </>
